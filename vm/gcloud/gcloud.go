...
func (jsonVM *jsonVM) toVM() *vm.VM {
	...
	if lifetimeStr, ok := jsonVM.Labels["lifetime"]; ok {
		if lifetime, err = time.ParseDuration(lifetimeStr); err != nil {
			vmErrors = append(vmErrors, vm.NoExpiration)
		}
	} else {
		vmErrors = append(vmErrors, vm.NoExpiration)
	}
	...
	if len(jsonVM.NetworkInterfaces) == 0 {
		vmErrors = append(vmErrors, vm.BadNetwork)
	} else {
...
func (o *providerOpts) CreateFlags(flags *pflag.FlagSet) {
	flags.StringVar(&o.MachineType, "machine-type", "n1-standard-4", "DEPRECATED - Use gcloud-machine-type instead")
	flags.MarkDeprecated("machine-type", "use "+ProviderName+"-machine-type instead")
	flags.StringSliceVar(&o.Zones, "zones", []string{"us-east1-b", "us-west1-b", "europe-west2-b"}, "DEPRECATED - Use gcloud-zones")
