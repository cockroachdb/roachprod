...
// init will inject the AWS provider into vm.Providers, but only if the aws tool is available on the local path.
func init() {
	if _, err := exec.LookPath("aws"); err == nil {
		vm.Providers[ProviderName] = &Provider{}
	} else {
		log.Printf("please install the AWS CLI utilities (https://docs.aws.amazon.com/cli/latest/userguide/installing.html)")
	}
}
...
// ConfigureCreateFlags is part of the vm.ProviderFlags interface.  This method sets up a lot
// of maps between the various EC2 regions and the ids of the things we want to use there.  This is
// somewhat complicated because different EC2 regions may as well be parallel universes.
func (o *providerOpts) ConfigureCreateFlags(flags *pflag.FlagSet) {
	...
	flags.StringVar(&o.RemoteUserName, ProviderName+"-user", "ubuntu", "Name of the remote user to SSH as")
}
...
// CleanSSH is part of vm.Provider.  This implementation is a no-op, since we depend on the user's
// local identity file.
func (p *Provider) CleanSSH() error {
	return nil
}

// ConfigSSH ensures that for each region we're operating in, we have a <user>-<hash> keypair,
// where <hash> is a hash of the public key.  We use a hash since a user
// probably has multiple machines they're running roachprod on and these machines (ought to) have separate
// ssh keypairs.  If the remote keypair doesn't exist, we'll upload the user's ~/.ssh/id_rsa.pub file
// or ask them to generate one.
	...
	for _, r := range regions {
		...
		g.Go(func() error {
			...
			if !exists {
				...
				log.Printf("imported %s as %s in region %s", sshPublicKeyFile, keyName, region)
			}
			return nil
		})
	}
...
// allRegions returns the regions that have been configured with AMI and SecurityGroup instances.
...
// allZones returns all AWS availability zones which have been correctly configured within the given region.
...
func (p *Provider) runInstance(name string, zone string, opts vm.CreateOpts) error {
	...
	if err := runJSONCommand(args, &data); err != nil {
		return err
	}
	log.Printf("created instances %+v", data)
	return nil
}
