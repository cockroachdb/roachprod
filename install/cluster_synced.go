...
func (c *SyncedCluster) Stop() {
	...
	c.Parallel(display, len(c.Nodes), 0, func(i int) ([]byte, error) {
		...
		// NB: the awkward-looking `grep` invocation serves to avoid having the
		// grep process match its own output from `ps`.
		cmd := "ps axeww -o pid -o command | grep -E 'ROACHPROD=[a-z]+' | awk '{ print $1 }' | xargs kill -9 || true;"
...
func (c *SyncedCluster) RunLoad(cmd string, stdout, stderr io.Writer) error {
	...
	return session.Run("ulimit -n 16384; env ROACHPROD=true " + cmd + " " + strings.Join(urls, " "))
}
...
func (c *SyncedCluster) Ssh(args []string) error {
	...
	if len(args) > 0 {
		allArgs = append(allArgs, "env", "ROACHPROD=true")
	}
	if c.IsLocal() {
